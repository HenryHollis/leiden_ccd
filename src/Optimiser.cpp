...
{
 size_t n = partition->get_graph()->vcount(); //number of nodes in graph
 vector<bool> is_membership_fixed(n, false); //vector of False's, none of the nodes are fixed yet
 return this->optimise_partition(partition, is_membership_fixed); //see method below
}
...
{
  vector<MutableVertexPartition*> partitions(1); //create vector of length 1
  partitions[0] = partition;   //set first element to be our MutableVertexPartition*
...
double Optimiser::optimise_partition(vector<MutableVertexPartition*> partitions, vector<double> layer_weights, vector<bool> const& is_membership_fixed, size_t max_comm_size)
{
  ...
  size_t nb_layers = partitions.size();  //length of partitions array, i.e. 1
  if (nb_layers == 0)
    throw Exception("No partitions provided.");

  // Get graphs for all layers
  vector<Graph*> graphs(nb_layers);  //create a vector "graphs" of graph pointers
  for (size_t layer = 0; layer < nb_layers; layer++) //populate the vector "graphs" with loop
  ...
  for (size_t v = 0; v < n; v++) {
    if (is_membership_fixed[v]) { //if membership is fixed
      fixed_nodes.push_back(v);   //push node to "fixed_nodes" vector
      fixed_membership[v] = partitions[0]->membership(v); //store the community that v is fixed into
    }
  }
